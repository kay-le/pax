"""Welfare Shaper (Attention-based) agent.

Uses the shaper_att architecture (single GRU + multi-head self-attention
over hidden states across environments) instead of MFOS's three-stream
GRU with explicit theta gating.

Key differences from MFOS-based WelfareShaper:
  - Standard MemoryState (hidden + extras only, no th/curr_th)
  - No meta_policy() — shaping is implicit via attention on hidden states
  - Standard Sample (no meta_actions field)
  - Network input is just observations (not (observations, meta_actions))

The welfare/Lagrangian objective is handled by the runner, not the agent.
"""
...
class WelfareShaperAtt(AgentInterface):
    """PPO agent with attention-based shaping (from scalable opponent shaping),
    designed for welfare maximisation with IR constraints."""
    ...
    def __init__(
        ...
        def policy(
            ...
            # Standard call — no (obs, meta_actions) tuple, just obs
...
def make_welfare_shaper_att_agent(
    args,
    agent_args,
    obs_spec,
    action_spec,
    ...
    """Create a WelfareShaperAtt agent using shaper_att networks."""
    ...
    if args.env_id == "coin_game":
        ...
        att_type = getattr(args, "att_type", "att")
        if att_type == "att":
        ...
        # Fallback: basic GRU + attention for IPD
        network, initial_hidden_state = make_GRU_ipd_att_network(
            action_spec, agent_args.hidden_size
        )
