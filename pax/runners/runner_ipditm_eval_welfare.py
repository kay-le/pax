...
class IPDITMEvalWelfareRunner:
    """
    Welfare evaluation runner for InTheMatrix (STORM). Mirrors IPDITMEvalRunner
    but additionally tracks joint welfare, individual returns, and IR
    (individual rationality) constraint satisfaction against pre-calibrated
    reference values (welfare.v_ref_shaper / welfare.v_ref_opponent).

    The runner can be used to run any two RL agents together either in a
    meta-game or regular game, it composes together agents, watchers, and
    the environment. Within the init, we declare vmaps and pmaps for training.
    Args:
        agents (Tuple[agents]):
    ...
    def run_loop(self, env_params, agents, watchers):
        ...
        # Load pretrained shaper params (frozen). Support both model_path /
        # run_path and the legacy model_path1 / run_path1 names.
        model_path1 = self.args.get("model_path1", self.args.get("model_path"))
        run_path1 = self.args.get("run_path1", self.args.get("run_path"))

        # Only call wandb.restore for online mode AND a wandb-relative
        # model path (absolute local paths cannot be restored).
        if (
            self.args.wandb.get("mode", "online") not in ["offline", "disabled"]
            and not os.path.isabs(model_path1)
        ):
            wandb.restore(
                name=model_path1,
                run_path=run_path1,
        ...
        pretrained_params = load(model_path1)
        a1_state = a1_state._replace(params=pretrained_params)
        print(f"Loaded pretrained shaper from: {model_path1}")
        print(f"v_ref_shaper:   {self.v_ref_shaper:.4f}")
        print(f"v_ref_opponent: {self.v_ref_opponent:.4f}")

        if self.args.get("model_path2", None):
            print("Loading Second Agent")
            model_path2 = self.args.model_path2
            if (
                self.args.wandb.get("mode", "online")
                not in ["offline", "disabled"]
                and not os.path.isabs(model_path2)
            ):
                wandb.restore(
                    name=model_path2,
            ...
            pretrained_params = load(model_path2)
        ...
        if watchers:
            ...
            # log the inner episodes (per-outer-step learning curve)
