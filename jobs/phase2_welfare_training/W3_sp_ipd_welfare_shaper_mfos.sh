...
#SBATCH --job-name=W3sp_ipd_welfare_mfos
...
SEED=${1:-0}
EXPERIMENT="ipd=welfare_shaper_v_tabular"
start_time=$(date +%s)
echo "Start W3-sp $EXPERIMENT (self-play ref): $(date '+%Y-%m-%d %H:%M:%S')"
...
python -m pax.experiment +experiment/$EXPERIMENT seed=$SEED \
    ...
    ++welfare.v_ref_shaper=<R1_r1> \
    ++welfare.v_ref_opponent=<R1_r2>

mkdir -p "$HOME/wandb_saved"
cp -r "$WANDB_DIR"/wandb/offline-run-* "$HOME/wandb_saved/" 2>/dev/null || true
end_time=$(date +%s)
echo "End W3-sp $EXPERIMENT: $(date '+%Y-%m-%d %H:%M:%S')"
