...
#SBATCH --job-name=W5_ipd_welfare_rl
...
SEED=${1:-0}
EXPERIMENT="ipd=welfare_shaper_rl_v_tabular"
start_time=$(date +%s)
echo "Start W5 $EXPERIMENT (IR calibration): $(date '+%Y-%m-%d %H:%M:%S')"
...
end_time=$(date +%s)
echo "End W5 $EXPERIMENT: $(date '+%Y-%m-%d %H:%M:%S')"
