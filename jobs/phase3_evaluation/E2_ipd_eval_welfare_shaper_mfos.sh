...
#SBATCH --job-name=E2_ipd_eval_welfare_mfos
...
SEED=${1:-0}
EXPERIMENT="ipd=eval_welfare_shaper_v_tabular"
start_time=$(date +%s)
echo "Start E2 $EXPERIMENT: $(date '+%Y-%m-%d %H:%M:%S')"
...
end_time=$(date +%s)
echo "End E2 $EXPERIMENT: $(date '+%Y-%m-%d %H:%M:%S')"
